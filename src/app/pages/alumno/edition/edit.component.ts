...
export class EditionComponent {
    ...
    constructor(private apiService: ApiService, private formBuilder: FormBuilder, private activactedRoute: ActivatedRoute, private router: Router) { }
    ...
    public saveAlumno(idAlumno?: number): void {
        ...
        if (this.alumnoNuevoForm.valid) {
            ...
            idAlumno ? this.apiService.updateAlumno(alumno).pipe(takeUntil(this.destroy$)).subscribe(() => { this.router.navigateByUrl(MODULES.ALUMNO + '/' + COMPONENTS.LIST) }) :
                this.apiService.addAlumno(alumno).pipe(takeUntil(this.destroy$)).subscribe(
                    () => {
