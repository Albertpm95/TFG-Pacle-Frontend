...
export class EditionComponent {
    ...
    private loadForm(idUsuario: string): void {
        this.apiService.getUsuarioID(idUsuario).pipe(takeUntil(this.destroy$)).subscribe((response: Usuario) => {
            this.usuarioDetailsForm = this.formBuilder.group({
    ...
    public save(idUsuario?: number): void {
        if (this.usuarioDetailsForm.valid) {
            ...
            idUsuario ? this.apiService.updateUsuario(usuario_nuevo).pipe(takeUntil(this.destroy$)).subscribe(() => { this.router.navigateByUrl(MODULES.USUARIO + '/' + COMPONENTS.LIST) }) : this.apiService.addUsuario(usuario_nuevo).pipe(takeUntil(this.destroy$)).subscribe(() => { this.router.navigateByUrl(MODULES.USUARIO + '/' + COMPONENTS.LIST) })
        }
    }
